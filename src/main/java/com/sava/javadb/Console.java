...
public class Console {
    ...
    private void showHelp() {
        ...
        System.out.println("SELECT <columns> FROM <table> [WHERE <column> <op> <value>] [LIMIT <count>] - Retrieve rows");
    ...
    private void handleSelect(SelectCmd select) {
        List<Row> rows = db.select(select.getCols(), select.getTableName(),
                select.getWhereCol(), select.getWhereOp(), select.getWhereVal(), select.getLimit());
        for (Row row : rows) {
